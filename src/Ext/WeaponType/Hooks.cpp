...
DEFINE_HOOK(6FF4DE, TechnoClass_Fire_IsLaser, 6) {
	...
	if(BuildingClass* pBld = specific_cast<BuildingClass*>(pThis)) {
		...
		if((pLaser = pBld->CreateLaser(pTarget, idxWeapon, pTWeapon, CoordStruct::Empty)) != nullptr) {
			...
			if (pBldTypeData->PrismForwarding.Enabled == BuildingTypeExt::cPrismForwarding::EnabledState::Yes
				|| pBldTypeData->PrismForwarding.Enabled == BuildingTypeExt::cPrismForwarding::EnabledState::Attack) {
				//is a prism tower
