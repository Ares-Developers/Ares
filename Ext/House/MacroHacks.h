...
#define CRAZY_MACRO_GO_AWAY_2(TypeName)                                                      \
	...
	for(int i = 0; i < TypeName ## TypeClass::Array->Count; ++i) {                             \
		...
		if(EarliestFrame < CreationFrames[i] || EarliestTypenameIndex == -1) {                   \
			...
			EarliestFrame = CreationFrames[i];                                                     \
                                                                                             ...
                                                                                             \
	int EarliestOdds = RulesClass::Global()                                                    \
		->get_FillEarliestTeamProbability()->GetItem(AIDiff);                                    \
	...
	if(BestChoices.size()) {                                                                   \
		int RandomChoice = ScenarioClass::Global()                                               \
			->get_Random()->RandomRanged(0, BestChoices.size() - 1);                               \
